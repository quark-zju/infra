...
| Any machine that can SSH to `opi` as root and read the vault secret | `ansible-playbook playbooks/opencode.yml --ask-vault-pass` | `opi` | Installs and manages the OpenCode and Kimi web services: the kimi npm package, service user, dotfiles/leash repos, Rust toolchain, config symlinks, `/etc/opencode/opencode.env`, systemd units, and running services. OpenCode runs from `/usr/local/bin/opencode`; if that binary is missing the playbook prints a hint to deploy it from `quark-zju/opencode` with `make clean deploy`. |
