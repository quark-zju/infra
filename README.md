...
| Any machine that can SSH to the `macos` inventory host | `ansible-playbook playbooks/macos.yml` | `mac` | Applies only the shared macOS role over SSH: installs Homebrew packages including Neovim, Zed, pyenv, ripgrep, and LinearMouse; configures mouse scrolling separately from trackpad scrolling; and installs the Hammerspoon config. It does not run the local dotfiles role. |
