...
| --- | --- | --- | --- |
| The machine you want to configure | `ansible-playbook playbooks/local.yml` | `localhost`, through the `local` inventory group | Always applies the local dotfiles role on macOS/Linux: clones or updates `~/src/dotfiles`, writes `~/.gitconfig.local`, writes dotfiles Cargo config into `~/.cargo/config.toml`, and manages dotfile symlinks including a shared AGENTS.md for opencode and Zed plus dedicated AGENTS.md files for Codex (`.codex/AGENTS.md`) and DSH (`.dsh/AGENTS.md`), `~/bin/scripts`, and (on Linux) desktop/Wayland configs under `~/.config` such as `sway`, `waybar`, and `fontconfig`. It also injects dotfiles' `.codex/config.toml.partial` snippet into `~/.codex/config.toml`. On macOS it also installs Homebrew packages including Neovim, Zed, Hammerspoon, pyenv, ripgrep, and JetBrains Maple Mono NF; configures GUI apps to inherit the current PATH when run with sudo access; disables natural scrolling globally; and installs the Hammerspoon config. Use `-K` if Ansible needs to prompt for the sudo password. On Linux it also applies the Linux hardening role with `become`. |
...
| Any machine that can SSH to the `macos` inventory host | `ansible-playbook playbooks/macos.yml` | `mac` | Applies only the shared macOS role over SSH: installs Homebrew packages including Neovim, Zed, Hammerspoon, pyenv, ripgrep, and JetBrains Maple Mono NF; configures GUI apps to inherit the current PATH when run with sudo access; disables natural scrolling globally; and installs the Hammerspoon config. Use `-K` if Ansible needs to prompt for the sudo password. It does not run the local dotfiles role. |
| Any machine that can SSH to `opi` as root and read the vault secret | `ansible-playbook playbooks/opencode.yml --ask-vault-pass` | `opi` | Installs and manages OpenCode and DeepSeek Harness; Kimi is disabled by default. OpenCode runs from `/usr/local/bin/opencode`; if that binary is missing the playbook prints a hint to deploy it from `quark-zju/opencode` with `make clean deploy`. It also disables Armbian `/etc/update-motd.d/` scripts (`10-armbian-header`, `15-ap-info`, `20-ip-info`, `30-armbian-sysinfo`, `35-armbian-tips`, `41-commands`) by removing their execute bit, cutting SSH login startup time and decluttering login output. |
| Same as above, with Kimi explicitly enabled | `ansible-playbook playbooks/opencode.yml --ask-vault-pass -e opencode_enable_kimi=true` | `opi` | Installs and manages the Kimi package and service in addition to the default services. |
| Same as above, skipping Kimi tasks entirely | `ansible-playbook playbooks/opencode.yml --ask-vault-pass --skip-tags kimi` | `opi` | Uses Ansible's tag skip feature to omit Kimi package, configuration, and service tasks. |
